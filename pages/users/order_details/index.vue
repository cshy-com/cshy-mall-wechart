  ...
  <view>
    <view class="order-details">
      <view
        ...
        v-if="(orderInfo.deliveryType == 'express' && orderInfo.status > 0)||showShip"
      >
      ...
      <view class="">
        ...
        <view class="wrapper">
          ...
          <template   v-if="(orderInfo.deliveryType == 'express' && orderInfo.status > 0)||showShip">
        
          <view
            class="item acea-row row-left"
        v-for="(item,index) in trackingNoList"
          >
            <view >快递单号{{trackingNoList.length>1?index+1:''}}：</view>
            <view class="conter" >{{ item || '' }} </view>
            <view
              class="copy copy-data"
        ...
        >
          ...
          <view class="bnt cancel" v-if="!orderInfo.paid&&orderInfo.status != 4" @click.stop="cancelOrder"
            >取消订单</view
          >
          <view @click="callPhone" class="bnt cancel">联系客服 </view>
          <view
            class="bnt bg-color"
            v-if="!orderInfo.paid&&orderInfo.status != 4"
          ...
          <navigator
            hover-class="none"
            :url="'/pages/users/goods_return/list?orderId=' + orderInfo.orderId"
            class="bnt cancel"
            v-if="[0, 1,2].includes(orderInfo.status) && orderInfo.paid"
            >申请退款
          </navigator>

          <view
            class="bnt bg-color"
            v-if="orderInfo.combinationId > 0"
            @tap="goJoinPink"
            >查看拼团</view
          >
...
export default {
  ...
  methods: {
    ...
    goLogisticsPage() {
      ...
      let trackingNo=this.trackingNoList.join(",")
      uni.navigateTo({
        url: `/pages/gift/gift_logistics?trackingNo=${trackingNo}&userMobile=${this.orderInfo.userMobile}`,
      })
    },
    /**
     * 获取订单详细信息
    ...
    getOrderInfo: function () {
      ...
      getOrderDetail(that.order_id)
        .then((res) => {
          ...
          let shipNums = res.data.orderInfoList.map((val) => ["",null,undefined].includes( val.shipNum)?0:val.shipNum);
      let payNums = res.data.orderInfoList.map((val) => val.payNum);
      ...
      if (shipNumsTotal>0&&(shipNumsTotal < payNumsTotal)&& res.data.refundStatus==0) {
        
            uni.$emit('updateTitle', '部分发货')
            this.showShip=true

          }else{
            uni.$emit('updateTitle', res.data.orderStatusMsg)
          }
         if(this.orderInfo.trackingNo){
      this.trackingNoList=this.orderInfo.trackingNo.split(',').filter(val=>val!='zp666') 
