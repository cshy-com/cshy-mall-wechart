<template>
	<div class="broadcast-details" :style="'height:'+windowH+'px'">
			...
			<scroll-view scroll-y="true" style="height: 100%; overflow: hidden;" :scroll-top="scrollTop" scroll-with-animation="true">
				...
				<div class="chat" ref="chat" >
					<template v-for="item in history">
			                ...
			                path: '/pages/admin/orderDetail/index?id=' + item.orderInfo.order_id
										...
										<div class="conter acea-row row-middle">
											<div class="broadcast-details_order noPad" v-for="(val, inx) in item.orderInfo.cartInfo" :key="val.id">
												<router-link class="broadcast-details_box noPad" :to="{ path: '/pages/goods/goods_details/index?id=' + val.product_id }" v-if="inx == 0">
													<div class="broadcast_details_img">
									...
									<div class="conter acea-row row-middle" v-if="item.msn_type === 5">
										<div class=" noPad">
											<router-link class="acea-row row-column-around noPad" v-if="item.productInfo.id" :to="{ path: '/pages/goods/goods_details/index?id=' + item.productInfo.id }">
									...
									<div class="conter acea-row row-middle" v-if="item.msn_type === 4">
										<img src="https://cshy.store/file/sysFile/wechart/signal2.gif" class="signal" style="margin-right: 0.27rem;" />12’’
									</div>
						...
						<div class="item acea-row row-top row-right" v-else :key="item.no">
		...
		<div class="footerCon" :class="active === true ? 'on' : ''" :style="'transform: translate3d(0,' + percent + '%,0);'" ref="footerCon">
			<form>
				<div class="footer acea-row row-between row-bottom" ref="footer">
					...
					<img @click="uploadImg" src="https://cshy.store/file/sysFile/wechart/plus.png" />
					<img :src="
              active === true
                ? 'https://cshy.store/file/sysFile/wechart/keyboard.png'
                : 'https://cshy.store/file/sysFile/wechart/face.png'
            "
					 @click="emoticon" />
				 ...
				 v-if="emojiGroup.length > 0">
					<block v-for="(emojiList, index) in emojiGroup" :key="index">
						<swiper-item>
							...
							<img src="https://cshy.store/file/sysFile/wechart/del.png" class="emoji-outer" />
						</swiper-item>
					</block>
		...
		<div class="recording" v-if="recording">
			<img src="https://cshy.store/file/sysFile/wechart/recording.png" />
		</div>
		<home></home>
	</div>
</template>
