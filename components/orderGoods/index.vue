<template>
	<view class="orderGoods">
		<view class='total' v-if="isShowOrderProNum">共{{ orderProNum?orderProNum:totalNmu}}件商品</view>
		<view class='goodWrapper pad30'>
			<view class='item acea-row row-between-wrapper' v-for="(item,index) in cartInfo" :key="index"
				...
				<view class='pictrue'>
					<image :src='item.image||item.mainImage'></image>
				</view>
				<view class='text'>
					...
					<view class='attr line1' v-if="item.sku">{{item.sku}} x {{item.payNum ? item.payNum : item.cartNum}}</view>
					<view class='money font-color' v-if="isShowPrice">￥{{item.price}}</view>
